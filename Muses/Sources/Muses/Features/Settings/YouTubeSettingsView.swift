...
struct YouTubeSettingsView: View {
    ...
    /// Cookie 阶段失败基本都源于 macOS 权限(完全磁盘访问/钥匙串)或
    /// 浏览器会话本身;给普通用户一条直达系统设置的路,避免反复折腾。
    ...
    var body: some View {
        // P2 — 普通模式:一张状态卡 + 一个主按钮;yt-dlp、权限、Cookie 等
        // 技术细节全部折叠进「高级」,避免普通用户在 YouTube 连接上困惑。
    ...
    /// 一键状态机:未连接先连 OAuth,成功后直接接续 Web 首页授权;
    /// 已连接但未开启时只给开启入口;已开启时唯一动作是刷新会话。
    @ViewBuilder
    ...
    /// 连接成功后立即接续 Web 首页独立确认;Cookie 复用仍必须由用户
    /// 在授权框中显式允许,不会静默开启。
