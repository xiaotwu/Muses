...
/// Phase 26 — Audio Nerd Mode 纯逻辑测试。
/// 仅测 `AudioInfoModel.rows` 与 `AudioDevice` 静态构造,不触 Core Audio / SwiftUI / 资源加载。
@Suite("Phase 26 — Audio Nerd Mode")
struct Phase26AudioNerdTests {

    private let unknown = tr("Unknown", "未知")

    // MARK: - nil / 全空快照 → 所有可空字段为 Unknown
    ...
    func youTubeSourceLossyMono() {
        ...
        #expect(byLabel[tr("Bit Depth", "位深")] == unknown) // YouTube 无位深
    ...
    // MARK: - AudioDevice 构造(纯值,不触 Core Audio)
